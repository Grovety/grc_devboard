...
int AudioRxSlot::begin(uint32_t sample_rate, IOPlan& io_plan, 
      int port_number, eBitWidth bit_width, eSlotType slot_type, eSlotMode slot_mode, eBitShift bit_shift) {
    int errors = 0;
    ...
    i2s_pdm_rx_config_t rx_pdm_cfg = {
        ...
        .slot_cfg = I2S_PDM_RX_SLOT_DEFAULT_CONFIG(i2s_bit_width, i2s_slot_mode),
        .gpio_cfg = {
            .clk = ws_pin_,
            .din = din_pin_,
            .invert_flags = {
    ...
    //     rx_pdm_cfg.clk_cfg.mclk_multiple = I2S_MCLK_MULTIPLE_384;
    // }
    //TODO:
    //rx_pdm_cfg.clk_cfg.dn_sample_mode = I2S_PDM_DSR_16S;
    rx_pdm_cfg.slot_cfg.slot_mask = i2s_slot_mask;
    rx_pdm_cfg.slot_cfg.slot_mode = i2s_slot_mode;

    // i2s_chan_info_t chan_info_;

    if (errors == 0) {
        TST_ESP_OK(i2s_new_channel(&chan_cfg, NULL, &rx_handle_));
        TST_ESP_OK(i2s_channel_init_pdm_rx_mode(rx_handle_, &rx_pdm_cfg));
        TST_ESP_OK(i2s_channel_enable(rx_handle_));
        // TST_ESP_OK(i2s_channel_get_info(rx_handle_, &chan_info_));
    }

    // DEBUG_PRINTF("chan_info_.id = %d\n", chan_info_.id);
    // DEBUG_PRINTF("chan_info_.role = %d\n", chan_info_.role);
    // DEBUG_PRINTF("chan_info_.dir = %d\n", chan_info_.dir);
    // DEBUG_PRINTF("chan_info_.mode = %d\n", chan_info_.mode);
...
size_t AudioRxSlot::read(void* fragment, size_t bytes) {
  size_t bytes_read = 0;
  i2s_channel_read(rx_handle_, fragment, (bytes / bytes_width_) * bytes_width_, &bytes_read, portMAX_DELAY);
  return bytes_read;
}
 ...
 * it is necessary to optimally configure the driver's DMA channel for the conditions of the task.
 * I.e., it is necessary to determine the number of blocks (number of descriptors) and the size of each block
 * (number of frames per block). These two parameters are required to configure the driver's DMA channel and
 * are calculated in plan() function. In turn, the number of DMA blocks and the size of each DMA block affect
 * the optimal transaction size of the read operation (see 'fragment time') and the size of the entire buffer
 * (see 'polling time'), which are also calculated in plan() function. So, according to the parameters of the task,
 * plan() function calculates the 4 parameters: the number of blocks of the driver's DMA channel (used to 
 * configure the driver), the number of frames in each such block (used to configure the driver), the size of the
 * portion of the read operation (corresponds to 'fragment time', but matching is optional exact; this size is
 * recommended to be used in read operations), the full size of the driver buffer (corresponds to 'polling time',
 * but the correspondence is not necessarily exact; max number of data that can be accumulated between two reads).

 * It is important to take into account here: the max block size of the DMA channel is 4092 bytes;
 * the DMA channel is focused on operations with 16-bit data (although we have an 8–bit interface);
 * frames must fit completely in each block;
 * it is necessary to get at least 2 descriptors (2 DMA blocks), and a lot descriptors will require a lot of
 * memory for themselves (there is an artificial upper limit of 64).
 */
...
#define PLAN_LAST_STAGE_ID        5
    ...
    while(stage <= PLAN_LAST_STAGE_ID) {
        ...
        dma_buffer_size = 0; 
        io_plan.polling_buffer_size = PARTS(polling_time_msec, fragment_time_msec) * read_fragment_size_x;
        ...
        if (dma_buffer_size) 
            io_plan.dma_desc_num = PARTS(io_plan.polling_buffer_size, dma_buffer_size);

        if (stage == PLAN_LAST_STAGE_ID)
            break;
        ...
        read_fragment_size_x = (stage < PLAN_LAST_STAGE_ID) ? read_fragment_size_x + bytes_per_frame_x : io_plan.read_fragment_size;
        continue;
    }
    
    if (io_plan.dma_desc_num < 2)
        io_plan.dma_desc_num = 2;
...
size_t AudioRxSlot::get_fragment_size() {
    return fragment_size_;
}

// Return polling buffer size
size_t AudioRxSlot::get_buffer_size() {
    return buffer_size_;
}
