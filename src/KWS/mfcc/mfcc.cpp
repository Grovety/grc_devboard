...
MFCC::MFCC(int numMfccFeatures, int frameLen)
    : numMfccFeatures(numMfccFeatures),
      frameLen(frameLen)
{
...
std::vector<float> MFCC::CreateDctMatrix(int32_t inputLength, int32_t coefficientCount)
{
    ...
    for (k = 0; k < coefficientCount; k++) {
        for (n = 0; n < inputLength; n++) {
            M[k*inputLength + n] = normalizer * riscv_cos_f32((static_cast<double>(M_PI))/inputLength * (n + 0.5) * k);
        }
    }
    return M;
}
...
void MFCC::MfccCompute(const int16_t *audioData, float *mfccOut, size_t max_abs)
{
    int32_t i, j, bin;
...
#elif KWS_GET_MEL_SPEC
    for (i = 0; i < NUM_FBANK_BINS; i++)
        mfccOut[i] = melEnergies[i];
#endif //
}
